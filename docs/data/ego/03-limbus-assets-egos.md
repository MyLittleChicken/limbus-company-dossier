...
### 3. `extractable` — 상시 추출 풀. 게임 사실과 일치한다
...
필드를 붙인 쪽은 `limbus-assets` 의 **「추출 시뮬레이터」** 이며, 로그와 실측이 맞는다 —
`21003` 평생 스튜(싱클레어)는 지금 `extractable` 이 없다.

#### 값은 게임의 상시 추출 풀과 일치한다 (2026-07-31 확인)

**처음에는 "게임 사실이 아니라 도구 사정" 으로 판정했으나 철회한다.** 게임의 추출 확률표와
대조하니 어긋나지 않았다.

| E.G.O | 수감자 | 등급 | `extractable` |
| --- | --- | --- | --- |
| `20403` 적안 · `20404` 적안(開) | 료슈 | TETH · HE | O · O |
| `20104` 차원찢개 · `20603` 차원찢개 | 이상 · 홍루 | HE · HE | O · O |
| `20707` 흉탄 · `20107` 흉탄 | 히스클리프 · 이상 | TETH · HE | O · O |
| `21203` 초롱 · `21004` 초롱 | 그레고르 · 싱클레어 | TETH · HE | O · O |
| `20503` 집행 · `20905` 집행 | 뫼르소 · 로쟈 | HE · HE | O · O |
| `20103` 소망석 · `20305` 소망석 | 이상 · 돈키호테 | TETH · TETH | O · O |
| `20909` 노을 속으로 | 로쟈 | ZAYIN | O |
| **`20509` 착영휘도** | 뫼르소 | HE | **X** |

**착영휘도가 `X` 인 것이 판정을 굳힌다.** 게임 화면 우상단에 `[EVENT]` 배지가 붙는
이벤트 E.G.O 라 상시 풀에 없다. `20909` 노을 속으로(2026-04-30)는 더 이르게 나왔는데도
`O` 다 — **시기가 아니라 배급 경로로 갈린다.**

같은 이름 계열에서도 갈린다. 차원찢개 3종 중 `21107`(오티스)만 `X` 다.

> **`extractable` = 상시 추출 풀 포함 여부.** 출처가 시뮬레이터용으로 붙였을 뿐
> 값 자체는 게임과 맞는다.
...
다만 **「상시 추출」이 더 정확하다.** 이벤트 한정으로 얻는 E.G.O 도 그 시점에는
「추출 가능」이었기 때문이다. 획득 경로 전체(상시 추출 · 이벤트 보상 · 시즌 · 기본 지급)를
구분하는 필드는 **E.G.O 관련 파일 어디에도 없다** — 인격은 `Personality_Get_Condition.json`
이 있는데 E.G.O 대응 파일이 없다.
...
| `extractable` | 없음 | **assets 단독** — 상시 추출 풀 |
...
1. `extractable` 은 **상시 추출 풀**이다. 「이벤트로 얻었는가」는 담지 않는다 — `20509` 착영휘도가 `X` 인 이유
