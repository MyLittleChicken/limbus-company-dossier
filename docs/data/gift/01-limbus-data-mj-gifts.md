...
계열이며, 이 필드는 그것을 죄악으로 되바꾼 값이다(회차 2에서 441/441 확인).

우리는 색 어휘를 그대로 둔다(`Gift.attributeType`, 02-data-model 원칙 3).
회차 3에서 **`limbus-assets` 의 되바꾸기가 4건 실패**함을 게임으로 확인했다 —
원본 색을 지킨 것이 값을 지켰다.
...
게임 화면으로 **다섯 건**을 확인했고 **전부 하드 난이도 전용**이었다.
...
| `9249` | 조그맣고 근사한 바이올린 | **하드** | `true` ✓ | 없음 ✗ |
| `9841` | C형 정리 요원 장비 세트 | **하드** | `true` ✓ | 없음 ✗ |

> **어느 한쪽도 완전하지 않다.** mj 가 놓친 것이 69건, assets 가 놓친 것이 6건이다.
> **합집합 122건이 정답이다** — mj 단독 6건 중 3건을 게임으로 확인했다(2026-07-31).

### mj 단독 6건이 한 계통이다

```
9427 마을을 지킬 작살  +  9428 고래의 심장            →  9212 모든 악의 끝
9706 슬픈 폐품  +  9707 반짝이는 폐품  +  9431 부서진 바이올린  →  9249 조그맣고 근사한 바이올린
```

**6건 중 5건이 서로의 합성 재료거나 결과물**이다. 하드 전용 합성 계통 하나를
assets 가 통째로 놓쳤다.
