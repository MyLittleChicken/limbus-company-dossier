...
| id | 기프트 | assets `affinity` | mj `attributeType` |
| --- | --- | --- | --- |
| `9038` | 환상 사냥 | `envy` (VIOLET) | **AZURE** (gloom) |
| `9111` | 생체 맹독 바이알 | `pride` (INDIGO) | **AMBER** (sloth) |
| `9404` | 갇힌 구더기 | `sloth` (AMBER) | **SHAMROCK** (gluttony) |
| `9707` | 반짝이는 폐품 | `wrath` (CRIMSON) | **AMBER** (sloth) |

**우리는 mj 쪽을 적재한다.** 게임에서 이 4건의 기프트 색을 보면 갈린다.
...
1. `affinity` 는 **죄악이다.** 원본 색은 mj `gifts_detail.attributeType` 뿐이며 **4건 어긋난다**
...
**`affinity` ↔ `attributeType` 4건 불일치.** 게임에서 `9038` 환상 사냥 · `9111` 생체 맹독
바이알 · `9404` 갇힌 구더기 · `9707` 반짝이는 폐품의 **색**을 보면 갈린다.

지금은 mj 쪽(색 원본)을 적재하고 있으므로, mj 가 틀렸다면 4건이 잘못된 색으로 나간다.
