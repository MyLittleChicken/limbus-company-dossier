...
**`summary` 218/598** 은 짧은 표기다. 다만 **키가 있는 것과 값이 있는 것이 다르다.**
...
**둘 다 우리가 쓰지 않는다.** `flavor` 는 상세 화면의 읽을거리가 된다.

`summary` 는 **로케일 파일 전반의 공통 필드**다(E.G.O 편 회차 8에서 확인).
`mechanics/BattleKeywords*.json` 은 거의 100 % 갖는 반면 패시브 계열은 선택적이며,
**게임의 패시브 상세 화면은 `desc` 전문을 보여주고 `summary` 는 쓰지 않는다.**
상태 아이콘 툴팁 같은 좁은 자리를 위한 필드로 보인다.
