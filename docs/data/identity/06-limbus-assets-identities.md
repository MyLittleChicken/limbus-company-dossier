...
`identities.json` 의 `tags` 와 `identity_tag_list.json` 이 유일 문자열 기준 같은 집합이므로
목록 파일은 **중복 정보**다.

> **정정 취소 2026-07-30 (회차 9).** 여기서 "회차 1 이 95항목이라 적었는데 실제 94항목"이라고
> 적었으나 **그 정정이 틀렸다.** `Set` 으로 비교해 유일 문자열 수를 raw 항목 수로 착각했다.
> 정확한 수치는 **raw 95항목 · 유일 문자열 94종 · 마크업 제거 후 93종** 이며, 회차 1이 맞았다.
> `"The Wild Hunt"` 가 완전 동일 문자열로 2번 들어가 95 → 94 가 되고,
> `"Jia Family"` 의 마크업/비마크업 쌍이 94 → 93 을 만든다.
