...
#### 한국어 원문과 게임 화면 (2026-07-31 확인)

`loc-ko` 의 `Passive_Ego.json` 이 같은 문장을 한국어로 갖고, **게임 화면에 그대로 나온다.**

```
2050911 「칼끝에 맺힌 술방울」  (착영휘도 · 20509)

  [검계 우두머리 뫼르소 전용 상시 효과]        ← [AlwaysUseEGOPassive2050911]

  기본 스킬의 공격 적중 시 대상에게 [출혈] 2 부여 (스킬당 1회)

  이 인격은 [출혈], [호흡]을 부여하는 인격으로 취급됨
  - 이 효과로 인해서 기본 스킬이 [출혈]과 [호흡]을 부여하는 스킬로 취급됨
```

> **게임이 직접 "~로 취급됨" 이라고 쓴다.** 조건부 기믹은 우리가 데이터에서 유도한
> 해석이 아니라 **게임이 명시한 규칙**이다.

두 번째 줄이 중요하다 — **인격만이 아니라 기본 스킬까지 그 기믹을 부여하는 것으로
취급된다.** 기프트 조건이 "해당 기믹을 부여하는 스킬 보유" 를 볼 때 이 인격이 걸린다.
