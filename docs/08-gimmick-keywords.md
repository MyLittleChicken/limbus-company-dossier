...
| `10312` 로보토미 E.G.O:: 사랑과 증오의 이름으로 (돈키호테) | `sinking` | **대체 스킬** — 변신 시 S1–S3이 다른 스킬로 교체된다 | ✓ | — |
...
### 5.5 세 번째 조건 유형 — 대체 스킬

`10312` 는 조건이 E.G.O도 기프트도 아니다. **스킬 자체가 교체된다.**
...
`keywords` 에는 침잠이 있는데 `keywordSkills`(그 키워드를 보유한 스킬 슬롯)에는 없다.
기본 S1–S3으로는 침잠을 주지 않기 때문이다.

원본이 이것을 스킬 목록에 담고 있다. `limbus-assets/identities.json` 의 `skillTypes` 에서
`num`(덱 매수)이 **0인 항목이 대체 스킬**이다.
...
코인 효과를 보면 부여 기믹이 실제로 갈린다.

```
1031203  아르카나 비트!!            [UnBrokenCoinOnSucceedAttack] Inflict 2 [Burst]     ← 파열
1031207  리버스드 아르카나 슬레이브    [UnBrokenCoinOnSucceedAttack] Inflict 2 [Sinking]   ← 침잠
```

특정 상태로 전환(변신)하면 S1–S3이 대체 스킬로 바뀌고, 파열 대신 침잠을 부여한다.
`altKeywords` 는 이 대체 스킬이 주는 기믹을 가리킨다.

**따라서 조건 유형은 셋이다** — `ego` · `gift` · `form`(대체 스킬).

이 유형은 큐레이션 없이도 **회차 3(`skills.json`)의 코인 효과에서 재구성할 수 있다.**
`num=0` 스킬을 모아 코인 효과의 `Inflict [상태]` 를 읽으면 된다. 다른 두 유형과 달리
파생 가능한 정보다.

### 5.6 `form` 은 드문 현상이 아니다 — 37건

`skillTypes` 의 `num=0` 을 전수 집계한 결과다.
...
mj `altKeywords` 는 이 중 **1건(`10312`)만** 기록한다. 필드가 불완전한 것이지 현상이
드문 것이 아니다. `identity_keyword_modifiers.json` 도 `form` 유형을 담지 않는다.

같은 축의 죄악 필드는 완전하다. `altSins` 는 **184/184에서 공식이 성립한다.**

```
altSins = (대체 스킬의 죄악) − (기본 스킬의 죄악)      불일치 0건
```

대체 스킬이 있어도 죄악이 기본과 겹치면 `altSins` 가 빈다(32건). 즉 mj는 죄악 축에서는
차집합을 정확히 계산해 두었으면서 기믹 축에서는 하지 않았다.

**기믹도 같은 방식으로 계산할 수 있다** — 대체 스킬의 코인 효과가 부여하는 상태에서
기본 스킬의 것을 빼면 된다. 회차 3에서 확인한다.
...
model IdentityComboGimmick {
  identityId    Int
  gimmick       Gimmick
  /// ego | gift | form — form 은 변신으로 스킬이 교체되는 경우(5.5)
  sourceType    String
  /// form 은 특정 대상이 없으므로 null
  sourceId      Int?
...
10312 사랑과 증오의 이름으로
  ...
  키워드 = 파열·충전, 변신 시 파열 대신 침잠    ③ {rupture, charge, sinking}
```
...
- ~~`form` 유형을 가진 인격이 몇이나 되는가~~ → **37건**으로 확인 (5.6)
- ❓ 37건 각각이 어떤 기믹을 추가로 얻는가. 대체 스킬의 코인 효과에서 계산 — 회차 3
- ❓ 5절 4건 외에 `ego`/`gift` 조건부 조합이 더 있는가. 두 출처가 서로를 못 덮는 것이
