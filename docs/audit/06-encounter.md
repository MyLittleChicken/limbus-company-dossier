...
> **해소됨 (2026-08-03).** 위 표는 감사 시점(2026-08-02)의 스냅숏이다. 재설계 후 실측은
> `encounter_target` 398 → **1,371**(top·wave·phase·battle 네 갈래를 배타적 `kind` 로 통합) ·
> `encounter_target_part` 354 → **1,302** · `encounter_part_resist` 3,540 → **11,800**
> (1,180부위 × 10축. 122부위는 원본에 `resists` 키 자체가 없음) · `canonical.enemy` 1,342 →
> **870**(적 전용) + `enemy_part` **466**행(별도 테이블로 분리) · `enemy_text` 4,026 →
> **2,610**. **「적 1,342종」이라는 이 절의 서술은 틀렸다** — 아래 2.4·3.2·6.1 이 밝히듯
> 그 1,342행 중 472행은 애초에 적이 아니라 부위였고, 그중에서도 부모 적을 `loc` 에서 찾을
> 수 없는 고아 6건은 결손으로 남겨 `enemy_part` 최종 행 수는 466이다. 상세는
> [`00-summary.md` 6.2·7절](00-summary.md).
...
결과적으로 `enemy` 테이블 1,342행 중 어떤 인카운터에서도 이름으로 도달되지 않는 것이
**942행(70.2%)** 이다.

> **정정** — 위 「942종」은 「942행」이라 써야 정확하다. 이 1,342행에는 적이 아니라
> 부위인 472행이 섞여 있었다(2.1 해소 참고). 적만 세면 870종이고, 부위는 별도
> `enemy_part` 466행으로 분리됐다 — 「적 1,342종」은 **「적 870 + 부위 466. 초판이 둘을
> 한 표에 섞어 담았다」**로 고쳐 읽는다.
...
| B4 | `canonical.enemy` | 1컬럼 테이블 · 인바운드 FK 0 · 이름 조인으로 도달 안 되는 적 942/1,342(70.2%). **해소됨(2026-08-03)** — 1,342행을 적 870 + 부위 466 으로 분리하고 `part_id` 조인을 추가했다(3.2절 해소 참고) | `pg_constraint` 실측 |
