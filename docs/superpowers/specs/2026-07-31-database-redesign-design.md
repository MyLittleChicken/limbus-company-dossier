...
빈 껍데기를 만든다. 같은 「없음」의 두 표현이다.

마스터북 인격 편 회차 11·12 가 이미 이렇게 적었다 — 「`loc-en` 에는 `a1c9p2` 가
없다. ko·ja 에만 있고 그것도 빈 파일이라 패치 잔재로 보인다.」 **문서에는 있으나
DB 에는 담기지 않는 사실**이며, `raw_file` 이 그것을 질의 가능하게 만든다.
