...
§3 의 `scope` 를 이 값으로 고쳤다 — `field` 는 6인이 아니라 **출격 7인**, `roster` 는
**편성 12인**, `slot` 은 1~5 가 아니라 **1~7**, 그리고 **`waiting` 을 더했다**
(「대기 인원 포함」 · 「대기 인원 제외」를 말하는 기프트가 있다).
...
- **효과 시뮬레이션을 하지 않는다.** 수량 · 배율 · 지속 · 적 쪽 효과는 뽑지 않는다. 엔진에 필요한 것은 「이 기프트가 아예 켜질 수 있나」뿐이다. **재 보고 지킨다** — 아래 참조.
...
경혈식 글레이브처럼 「발동은 하되 크기가 0」인 기프트가 있다. 크기를 안 담으면 그런
기프트가 `fireable=true` 로 남아 팩 점수의 `F`(개수)를 부풀린다. 그래서 크기 칸을
넣을지 재 봤다.

**편성 수에 비례하는 기프트는 15건**이고, 그중 **11건은 고정분이 있어 크기가 0이 되지
않는다.**

```
생존의 이정표   (80 + 인격 수 × 5)   ← 80 고정      전격부      파열 1 부여
깨어진 대검     참격 합 위력 +2                     거울 속의 꽃  파열 위력 3
듀엘교본 3권    관통 합 위력 +2                     물 속의 달   호흡 위력 1
앙갚음 장부     타격 합 위력 +2                     피로 된 살점  출혈 3 부여
짧은 케인 소드   방어 레벨 감소 2                    애달픈 날숨   크리티컬 +80%
C형 정리 요원   스킬 1 합 위력 +3
```

나머지 넷 중 셋은 크기 문제가 아니다 — 데스페라도는 기본 절이 없어 2인 미만이면 애초에
안 켜지고, 누군가 놓쳐버린 사원증은 이미 §0 의 결손이며, 만화경은 「자신과 동일한 소속
인격이 존재할 경우」라 6인 편성이면 대개 선다.

**남는 것은 경혈식 글레이브 하나뿐이고, 그마저 크기가 아니라 문턱값으로 표현된다.**

```
「모든 아군이 주조된 경혈을 (편성된 수 - 2)만큼 얻음 (최대 3)」
  = 혈귀가 3명은 있어야 1을 준다

cond:  unit_keyword/BLOODFIEND · op=gte · threshold=3 · scope=roster
```

**0개를 주는 것은 안 주는 것이다.** 「켜지되 0」보다 「안 켜진다」가 사실에 가깝다.
크기 칸을 넣으면 모형이 한 단계 깊어지는데(피로 된 살점 한 절에만 크기가 셋 —
혈찬 `×20` · 출혈 고정 3 · 피해 감소 `×10%`) 실익이 없다.

이 측정은 점수 모형 PR 의 입력으로 남긴다. 크기가 필요해지면 위 15건 목록과
「11건은 고정분 있음」이 그대로 출발점이다.

## 3. 데이터 모형

문단 하나가 능력 하나다. 능력마다 조건이 붙는다.

### 절은 네 종류다 (2026-08-11 사용자 확정)

설명문의 문단을 읽는 규칙이다. 이 넷이 `unconditional` · `conds` · `refines` 세 칸으로 담긴다.

```
기본 효과   조건 없이 돈다                      unconditional=true · conds 없음
            거울 속의 꽃 「매 턴 시작 시 모든 적에게 파열 위력 3 부여」

조건 효과   기본이지만 공급 조건이 붙는다          unconditional=false · conds 있음
            전격부 「충전 횟수를 소모하는 스킬 적중 시」 — 충전 인격이 있어야 한다

추가 효과   「- N인 이상」 티어                  unconditional=false · conds 에 threshold
            **미달이어도 앞의 기본·조건 절은 그대로 돈다.** 티어는 발동 조건이 아니다

배수 효과   크기가 (편성 수 × k)                conds 의 threshold 로 표현한다
            경혈식 글레이브 「(편성된 수 - 2)만큼」 → threshold=3
```

**「- N인 이상」 티어는 `refines` 가 아니라 독립 능력이다.** `refines` 는 「원 능력이
죽으면 같이 죽는다」인데, 티어는 원 능력과 독립으로 켜지고 꺼진다. 데스페라도가 이것을
증명한다 — 기본 절이 아예 없고 최저 티어가 2인이라, 원 능력이라 부를 것이 없다.

```
9235 데스페라도
  ordinal 0   cond: axis/BULLET · gte 2 · scope=roster
  ordinal 1   cond: axis/BULLET · gte 5 · scope=roster
  ordinal 2   cond: axis/BULLET · gte 8 · scope=roster
  → 탄환 2명 미만이면 독립 능력이 하나도 안 서므로 죽는다. 그것이 옳다

9803 거울 속의 꽃
  ordinal 0   unconditional=true                     ← 매 턴 파열 3
  ordinal 1   cond: axis/BREATH · gte 6              ← 티어
  ordinal 2   cond: axis/BREATH · gte 10             ← 티어
  → ordinal 0 이 무조건이므로 언제나 켜진다. 티어는 좋아지게 할 뿐이다
```

**`refines` 는 「효과가 강화되어」에만 쓴다.** 9073 엔도르핀 키트처럼 앞 절의 결과를
전제로 하는 것이다.

**「효과가 변경되어」는 강화가 아니라 대체다.** 조건은 같고 주는 효과만 갈린다 —
9123 넝마의 「해당 인격이 빼기 코인 스킬을 사용할 경우, **효과가 변경되어** …」가 그것이다.
발동 판정에서는 원 절이 이미 서므로 **대체판을 따로 담지 않는다.** 조건이 같은 절을
둘로 쪼개면 OR 로 읽혀 판정이 헐거워진다. 어느 효과가 나가는지는 크기의 문제이고
이 스펙의 비목표다(§2).

### 공명은 이미 갈려 있다

`trigger_ref.resonance_mode` 가 `activate`(일반 공명 7건) · `absolute`(완전 공명 8건)를
구별한다. 새 칸이 필요 없다 — `GiftAbilityCond` 가 이 칸을 그대로 이어받는다.

공명은 **스킬 슬롯에서 같은 속성이 얼마나 연속인가**다(2026-08-11 사용자 확정).
완전 공명은 연속 3개 이상이고, 연속 3개 이상인 묶음이 여럿이면 그 합이 수치다
(분노×3 · 오만 · 분노×3 → 분노 6 완전 공명). 따라서 **상한은 출격 인원 중 그 속성
스킬을 가진 수**이고, `have < need` 로 배제하는 지금 판정이 옳다.
...
model GiftAbilityCond {
  ...
  threshold Int?
  /// 세는 모집단. **거울 던전은 편성 12인 · 출격 7인 · 대기 5인이다**(§0).
  ///
  ///   field    출격 7인.        「출격 인원을 기준으로 함」(9283 상납된 시가)
  ///   roster   편성 12인.       「편성 인원을 기준으로 함」(9282 날개 모양 양초)
  ///                            「대기 인원 포함」(9212 모든 악의 끝)도 같다
  ///   waiting  편성 − 출격 5인.  「대기 인원에 …」(9778 통상 작전용 장비)
  ///                            「대기 인원 제외」(9795 떨어진 한 방울)는 field 다
  ///   slot     특정 자리
  ///   enemy · none
  ///
  /// 설명문이 분모를 직접 말한다. 넷을 구별하지 못하면 「3인 이상」이
  /// 분모에 따라 서기도 하고 안 서기도 한다.
  scope     String
  ...
  ///
  /// **refKind 마다 세는 표가 다르다.** 옛 판정이 빈 표를 보고 있었다.
  ///   axis           identity_axis (tag) · coin_token (skill)
  ///   association    identity_association
  ///   unit_keyword   identity_unit_keyword   ← identity_keyword 가 아니다.
  ///                  후자는 축 7종뿐이라 BLOODFIEND 가 없다
  ///   coin=minus     skill_stage.coin_value < 0 — 코인 위력이 음수인 스킬.
  ///                  8인격이 갖는다. 지금 엔진은 이 표를 보지 않아 언제나 0이다
  supply    String
  /// scope='slot' 일 때 자리 번호 **1~7**. 아니면 null.
  ///
  /// 출격이 7인이므로 7번 자리가 있다 — 9759 불 꺼진 랜턴이
  /// 「[편성 7번 인격 전용 효과]」다. 「편성 N번」은 곧 「출격 N번」이다(§0).
  slot      Int?
...
공급(supply, scope)
  ...
  supply=any    둘 중 큰 쪽
  refKind 마다 세는 표가 다르다 — association 은 identity_association,
  unit_keyword 는 identity_unit_keyword, coin=minus 는 skill_stage.coin_value<0
...
### 검수 절차 (2026-08-11 사용자 확정)

**두 단계를 왕복한다.** 사람이 판단을 내리고 기계가 고친다 — 반대가 아니다.

```
1차   사용자가 직접 보고 상호작용한다. 판정은 「맞다 / 틀리다」 둘뿐이다.
      이유를 적지 않는다 — 456건을 빠르게 훑는 것이 이 단계의 일이다.

2차   「틀리다」로 표시된 것만 모아 **에이전트가 먼저 읽고**, 사용자가
      무엇이 틀렸는지 서술한다. 서술은 그 건의 기록에 남는다.

3차   에이전트가 저작을 고치고, 고친 건은 다시 미판정으로 돌아가 1차를
      한 번 더 받는다. 「틀리다」가 없어질 때까지 반복한다.
```

1차를 이진으로 묶는 것이 요점이다. 「무엇이 틀렸나」를 456번 적게 하면 회차가
끝나지 않는다. 이유는 틀린 것에만, 그것도 에이전트가 먼저 읽은 뒤에 적는다.

**판정 상태는 넷이다** — `pending`(아직 안 봄) · `ok`(맞다) · `bad`(틀리다,
서술 대기) · `revised`(고쳤으니 다시 1차로). `revised` 는 곧바로 `pending` 이
되므로 저장하지 않고 회차 수만 센다.

### 검수 단위

50 기프트씩 한 회차. 총 10회차 안팎. 회차마다 확정분을 `gift-ability.jsonl` 에 커밋한다. 마스터북 51회차와 같은 방식이다. 진행 상황은 `gift-ability.progress.json` 이 기억하고 **함께 커밋한다** — 회차가 여러 날에 걸치고 기계가 바뀔 수 있다.
...
scope 가 field · roster · waiting · slot · enemy · none 여섯 중 하나다
scope='slot' 이면 slot 이 1~7, 아니면 null
...
**덱은 편성 12인 · 출격 7인으로 짠다.** 위 두 덱은 6인이라 옛 모형이다 — 7번 자리가 없고
대기 분모가 공집합이라, 그 둘에 걸리는 조건을 아예 시험하지 못한다(§0).

```
덱 C · 결손 여섯이 죽는지 (편성 12 · 출격 7 · 그 소속을 하나도 안 넣는다)
  9246 누군가 놓쳐버린 사원증  → 발동 불가   (지금: 가능)  절 둘 다 소속
  9778 통상 작전용 장비        → 발동 불가   (지금: 가능)  절 둘 다 림버스 컴퍼니
  9271 흑수환 - 묘            → 발동 불가   (지금: 가능)  절 둘 다 묘
  9843 경혈식 글레이브          → 발동 불가   (지금: 가능)  혈귀 3명 문턱
  9262 모든 것의 뼈대          → 발동 불가   (지금: 가능)  절 둘 다 약지
  9268 모든 것의 본능          → 발동 불가   (지금: 가능)  절 둘 다 약지

덱 D · 분모 셋을 가른다 (편성 12 · 출격 7 · 대기 5)
  피쿼드호 3인을 **대기에만** 넣는다
  9212 모든 악의 끝    → 발동 가능   「대기 인원 포함」이므로 roster 로 센다
  9795 떨어진 한 방울  → 발동 불가   「대기 인원 제외」이므로 field 로 센다
  분모를 안 가르면 둘이 같은 답을 낸다 — 이 덱이 그것을 잡는다

덱 E · 자리 7번 (편성 12 · 출격 7)
  9759 불 꺼진 랜턴   → 발동 가능   「[편성 7번 인격 전용 효과]」
  slot 을 1~5 로 두면 7번 자리가 없어 죽는다
```

**덱 C 여섯은 이 모형의 존재 이유다.** 게이트 PR(#33)이 결손으로 넘긴 것이고,
...
**한 방향만 세면 안 된다.** 게이트 PR 에서 「죽는 기프트 178 → 130」을 「48건 고쳤다」로
읽었는데, 실제 뜻은 「48건이 죽음에서 삶으로 **이동했다**」였고 그중 몇이 살 자격이
있는지는 아무도 묻지 않았다. 규칙을 느슨하게 할 때는 **사라진 죽음이 아니라 새로 생긴
삶을 센다.** 옮겨간 전부를 목록으로 뽑아 하나씩 자격을 확인한다.

### 넓은 덱 검증 (`scripts/audit-fireable-wide.ts`)

163덱 × 451기프트로 교차 신호를 낸다. 이 스크립트는 게이트 PR 재검증에서 사람 눈이
놓친 과대 판정 둘(9262 · 9268)을 잡았다 — **자동 신호가 눈보다 넓다.**

```
             지금(게이트 모형)   절 모형이 도달해야 할 값
S1 과대           16                 0
S2 과소            0                 0        지금도 0이다. 늘면 회귀다
S3 무반응         94                 크게 줄어야 한다
                                    (92 켜짐 · 2 죽음 — 편성 참조가 있는데
                                     163덱 판정이 같다 = 조건이 구실을 못 한다)
```

**S2 = 0 을 지키는 것이 최우선이다.** 과소 판정은 사용자 목록에서 기프트가 사라지는
것이고, 지금 모형이 그 점에서는 깨끗하다. S1·S3 을 줄이려다 S2 를 늘리면 손해다.
...
- ~~**검수 회차를 누가 도나.**~~ **닫혔다** (2026-08-11 사용자 확정) — **456건 전건을 사람이 본다.** 두 판 일치분을 표본으로 넘기지 않는다. §5 가 이미 그렇게 적혀 있고, 계획은 이 부담을 전제로 짠다: 회차는 재개 가능해야 하고(중단 지점을 파일이 기억한다), 한 기프트당 화면 하나로 끝나야 하며, 확정분은 회차마다 커밋되어 되돌아갈 일이 없어야 한다.
...
- **혈찬 공급을 무엇으로 세나.** `identity_unit_keyword.BLOODFIEND` 는 라만차랜드 5인 전원을 담아 「혈귀 인격」에 정확하다. 그런데 기프트 조건은 「혈찬을 **소모하는 스킬**을 보유한 인격」이라 스킬 수준이다. `coin_token` 쪽은 인격별 토큰명으로 흩어져 있어(`BloodDinner*` · `BloodScissorPersonalityFirst` · `BloodyHandGregFirst` · `BloodArmorMeursault`) 통합 개념이 없고, 공주 로쟈는 `coin_token` 에 잡히지 않는다. 지금은 `unit_keyword` 로 세면 다섯이 다 잡히므로 그것을 쓴다. 「소모」와 「보유」가 갈리는 기프트가 나오면 그때 다시 본다.

### 이번 회차가 지운 질문

물어보지 않고 데이터에서 답한 것들이다. 다시 묻지 않는다.

```
공명 연속성      trigger_ref.resonance_mode 가 activate · absolute 로 이미 가른다
빼기 코인        skill_stage.coin_value < 0 · 8인격. 여우비·와일드헌트 히스클리프 포함.
                엔진이 이 표를 안 보고 있을 뿐이다
혈귀 어휘        identity_unit_keyword 에 BLOODFIEND 5인 — 결손이 아니었다.
                내가 identity_keyword(축 7종)를 본 탓이다
주살【신속】      흑수-오 필두 이상 · 홍원 군주 홍루
「자신에게 화상」  흑수-유 히스클리프·싱클레어가 COMBUSTION 태그를 이미 갖는다.
                혈염도의 결손은 태그가 아니라 need=null 이다
[편성 N번]       범위라는 별개 개념이 아니다. 그냥 조건이고 scope=slot 으로 담긴다
각력【묘】        흑수-묘 소속이 보유한 패시브 — 소속 조건이다
```
