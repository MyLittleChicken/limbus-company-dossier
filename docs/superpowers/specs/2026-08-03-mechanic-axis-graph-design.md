...
## 22. 4단계 — 평가기 `lib/engine/v2` (2026-08-04)

16절 흐름의 1·2·3·4·6·7 단계를 구현했다. 5단계(팩 점수화)는 18절이 「기대 효용의
저울추」를 범위 밖으로 두었으므로 남긴다 — 등급까지는 나오고 가중은 아직이다.

```
types.ts     입출력.  게임 지식이 없다 — 있으면 잘못 놓인 것이다
profile.ts   1·2단계  조건부 축 거르기 · 분모 셋을 한 번에 센다
evaluate.ts  3·4단계  참조 판정 · 등급 A/B/C
chain.ts     6단계    연쇄 깊이 2 · visited
load.ts      canonical 읽기.  **DB 는 이 파일에만 있다**
```

**레거시 `lib/engine` 은 건드리지 않았다.** 그쪽은 `public` 스키마의 토큰을
TypeScript 사전(`vocab.ts` 500줄)으로 옮기는 저작층이고, 이쪽은 `canonical` 이 이미
구조로 들고 있는 것을 읽기만 한다.

### ① 실측 — 편성 6인 화상·진동 덱

```
등급    A 146 · B 219 · C 86        (451)
전부 충족 A                95
  그중 확정                50        나머지 45 는 「가능」이다
평가 시간                 1.6ms      기프트 451 · 참조 전수
연쇄(9095 고장난 나침반)    31        1홉 31 · 2홉 0
```

### ② 결정 2 의 62/291/98 과 다르다 — 이유가 있다

설계는 A 62 · B 291 · C 98 로 예측했다. 실측은 146/219/86 이다. 원인은 **`roster_gated`
를 판정 가능으로 셌기 때문**이다. 설계의 A 는 「전부 roster/always」였다.

`roster_gated` 를 판정 불가로 두는 것은 정보를 버리는 일이다. 「화상 스킬 사용 시」는
편성에 화상 인격이 **없으면 절대 안 켜진다** — 이것은 확정이고, 사용자가 가장 알고
싶은 답이다. 그래서 판정에 넣되 **충족 쪽만 「가능」으로 내린다.**

```
불충족   언제나 확정   편성에 없으면 전투 중에도 안 생긴다
충족     roster        확정
         roster_gated  가능 — 그 턴에 실제로 그 스킬을 써야 한다
```

`Reason.certainty` 와 `GiftVerdict.certain` 이 그것이다. 뭉치면 「켜진다」로 읽혀
과대 판정이 된다 — 95 와 50 의 차이가 그 크기다.

**결정 2 의 취지는 유지된다.** 결합을 접지 않고 「N개 중 M개 충족」을 그대로 낸다.
B 가 가장 크다는 예측도 맞았다(219/451).

### ③ 연쇄를 반대로 쓴다

20절이 잰 것처럼 이 그래프는 희소하지 않다 — 밀도 63% · 차수 중앙값 305. 그래서
「무엇이 켜질 수 있나」로 쓰면 거의 전부가 나와 쓸모가 없다.

**「내가 든 기프트가 아직 안 켜진 기프트를 켜 주는가」만 본다.** 편성으로 이미 충족된
참조는 사슬에서 뺀다. 그러면 나침반(9095)이 31건을 낸다 — 사람이 읽을 크기다.

같은 조건에서 진혼(9088)은 **0건**이다. 진혼이 거는 화상을 이 편성이 이미 갖고
있어서다. 이것이 옳은 답이며, 안 거르면 화상 기프트 전부가 다시 나온다.

`consume` 은 사슬을 잇지 않는다 — 없애는 것이다.

### ④ 분모 분기가 실제로 산다

같은 임계 6 이 분모에 따라 갈린다.
...
진혼은 출전 분모라 대기 인원을 채워도 `have` 가 6 그대로다. 골든이 이것을 고정한다.

### ⑤ 테스트 33건

```
evaluate.test.ts  15   분모 · 조건부 축 · 임계 · 다단 · 등급 · 공명 · 배치 · certainty
chain.test.ts      8   깊이 · 자기 루프 · 상호 쌍 · 중복 합산 · consume · 이미 충족
golden.test.ts    10   실제 DB.  진혼 발동 · 피안개 미달 · 등급 실측 · 연쇄
```

순수 함수 셋은 DB 없이 돈다. 골든만 `canonical` 을 읽는다.
