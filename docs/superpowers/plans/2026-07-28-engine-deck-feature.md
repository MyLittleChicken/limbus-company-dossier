...
**정밀화 범위를 전수로 셌다.** `COUNT_AFFILIATION` 이 만들어지는 것은 기프트 74종 · 토큰 78건이고,
그중 인원수 근거를 찾은 것이 19토큰(18종), 판정 범위 표지를 찾은 것이 16토큰이다. 계획 근거 절의
숫자가 그대로 재현됐다.

| | 토큰 | 비고 |
| --- | ---: | --- |
| `COUNT_AFFILIATION` 전체 | 78 | 기프트 74종 |
| 인원수 근거를 찾음 | 19 | 그중 9712 의 2토큰은 합집합이라 일부러 근사 유지 |
| 인원수가 실제로 3에서 바뀜 | 4 | 2인 3건(9263·9264·9792) · 4인 1건(9220) |
| 판정 범위 표지를 찾음 | 16 | 편성 2 · 출전 14 |
| 근사가 그대로 남음 | 59 | 인원수 기준. 범위 기준으로는 62 |

**인원수와 판정 범위는 별개로 갈린다.** 9270·9220·9829 는 인원수는 읽히지만 범위 표지가 없어
범위만 근사를 유지한다. `docs/06` 10절이 이 셋을 "인원수 근사를 유지하는 3건" 으로 적고 있었는데
실측과 어긋나 정정했다 — 9220 은 실제로 `4인 이상` 을 읽는다.

**9712 합집합을 재현하고 막았다.** 고치기 전에는 `검계 4명 AND 흑운회 4명` 이 나왔다. 소속 토큰이
둘인 기프트는 실측으로 9246·9712·9713·9782 넷이고 인원수를 말하는 것은 9712 뿐이다. 골든 회귀는
이 수정으로 움직이지 않는다 — 화진 덱에도, hard 3층 후보 27종에도 9712 가 없다.

**`scope` 를 선택 필드로 되돌렸다.** 구현이 `'deck' | 'deployed' | 'unknown'` 필수 필드로 두어
`Condition` 이 좁아져 있었다. Global Constraints 가 금지한 것이고, `evaluate` 의 인자 타입이 함께
좁아진다. 없는 것이 곧 "표지가 없었다" 는 뜻이다.

**`statusKeyOf` 와 `STATUS_MATCH` 를 `vocab.ts` 로 옮겼다.** `load.ts` 에 있으면 그 표의 단위
테스트가 `@/lib/db` 를 거쳐 `PrismaClient` 를 끌고 온다 — 데이터베이스와 무관한 문자열 판정이다.
`load.test.ts` 는 `status-key.test.ts` 로 옮겼다.

**Task 1 배선은 웹 브랜치가 가져갔다.** `feat/recommendation-system` 이 먼저 올라갔다. `package.json`
의 `test` 는 문자열이 같아 충돌해도 자명하지만, `ci.yml` 의 「단위 테스트」 단계는 주석이 달라 병합
시 **양쪽 주석을 합쳐야 한다** — 한쪽을 버리면 다른 계층의 테스트가 왜 도는지 설명이 사라진다.
