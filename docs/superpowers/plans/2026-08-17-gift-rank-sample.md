...
겹침이 딱 6(공통 여섯)이 아니라 8까지 여는 이유가 있다. **거울 던전에 5등급은
2개, EX 는 2개뿐이다**(실측 2026-08-17). 공통 여섯이 그중 하나(9083 달의 기억,
5등급)를 이미 쓰므로, 「덱마다 EX 를 하나」를 채우려면 남은 2개를 세 덱이
나눠 써야 하고 최소 한 번은 겹친다 — 비둘기집이라 코드로 못 없앤다.

**그 이상 겹치면 코드 문제다.** 실측으로 덱 쌍마다 9장이 겹쳤는데, 그건 자리
메우기가 피할 것으로 되돌아가며 희귀 등급을 돌려 썼기 때문이고 위 ③단계
수정이 그것을 막는다.
...
- 설명문 전문이 줄바꿈째로 보인다
- **「None」이 한 장에도 안 보인다** — `grep -c 'None' /tmp/rank.html` 이 0 이어야 한다
- **`dragstart` 가 `setData` 를 부른다** — 없으면 Firefox 에서 끌기가 시작 안 된다
