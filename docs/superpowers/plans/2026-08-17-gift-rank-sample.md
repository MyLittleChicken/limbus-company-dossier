...
/** 이 기프트가 어느 칸에 드는가. 칸마다 고르게 뽑는다 */
function cellOf(c: GiftCard, supply: DeckSupply): string {
	const tier = c.tier === null ? 'EX' : String(c.tier);
	const fit = fitOfKeyword(c.keywordId, supply);
	const kw = fit > 0 ? '일치' : (c.keywordId === null || c.keywordId === 'None' ? '없음' : '불일치');
	return `${tier}|${kw}|${c.exclusive ? '전용' : '공용'}|${c.fireable ? '켜짐' : '죽음'}`;
}

/**
 * 칸을 돌아가며 하나씩 집는다(라운드 로빈).
 *
 * 칸이 많고 자리는 20 이므로, 한 칸에서 여럿 집기 전에 모든 칸이 한 번씩
 * 기회를 갖는다 — 그래서 한 등급에 몰리지 않는다.
 */
export function pickTwenty(
	...
	for (const id of shared) {
		const c = byId.get(id);
		if (c !== undefined && !taken.has(id)) {
			picked.push(c);
			taken.add(id);
		}
	}

	const cells = new Map<string, GiftCard[]>();
	for (const c of [...pool].sort((a, b) => a.giftId.localeCompare(b.giftId))) {
		if (taken.has(c.giftId)) continue;
		const k = cellOf(c, supply);
	...
	while (picked.length < WANT) {
		...
		for (const k of keys) {
			...
			const list = cells.get(k) as GiftCard[];
			const c = list[round];
			if (c === undefined || taken.has(c.giftId)) continue;
			picked.push(c);
			taken.add(c.giftId);
			addedThisRound = true;
		}
		if (!addedThisRound) break; // 못이 말랐다
		round += 1;
	}
	return picked;
}
