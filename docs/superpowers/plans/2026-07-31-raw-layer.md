...
2. 1,664파일 중 누락이 0이다                        ✔ 1,648 + 빈 파일 16 = 1,664
3. 출처 대조를 SQL 로 할 수 있다                     ✔ hardOnly 불일치 65건 재현
4. 현행 파이프라인이 그대로 돈다                      ✔ schema:validate · public.gift 456행
5. 스냅샷을 다시 넣어도 결과가 같다                    ✔ v2:load 두 번 · version 유지
```

### 실행 결과

```
npm test          135건 통과 (신규 v2 33건 + 기존 lib 102건)
npm run typecheck  통과
npm run v2:load    파일 1,664 · 개체 43,270 · raw_file 1,664 (빈 16) · 4.4초
npm run v2:verify  검사 13건 전부 통과
```

### 계획과 달라진 것

**Task 7 이 실행 중에 추가됐다.** 검증에서 「고유 srcPath 1,648 / 1,664」가 걸려
빈 파일 16개가 흔적을 못 남긴다는 설계 구멍이 드러났다. `raw_file` 테이블로 해소.

**DDL 이 `raw` 스키마만 만든다.** 계획은 셋 다 기대했으나 Prisma 는 모델이 있는
스키마만 `CREATE SCHEMA` 를 낸다. `canonical`·`app` 은 계획 2·8 에서 생긴다.
