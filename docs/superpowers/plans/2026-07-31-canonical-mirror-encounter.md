...
### 실행 후 정정 5건 (2026-07-31)

계획 7만 Task 3·4를 「Step 1–5」로 뭉뚱그렸다. 계획 2–6은 태스크마다 테스트
코드를 실제로 적어뒀고 실행이 그 위를 따라갔는데, 계획 7은 따라갈 것이 없어
규율이 무너졌다. 다섯을 고쳤다.

| # | 문제 | 고침 |
| ---: | --- | --- |
| 1 | `mirror.ts` · `encounters.ts` 에 테스트가 없었다 | 20건 추가. 변환기 12개 전부 TDD |
| 2 | `waves`·`phases`·`battles` 가 **JSON null** 로 들어갔다 | 키를 빼 SQL NULL 로. 0 → 192행 |
| 3 | 이름 빈 적 1건이 조용히 버려졌다 | 담고 결손 기록. target 397 → 398 |
| 4 | 계획 7 계열 한국어 결손이 하나도 기록 안 됐다 | `field_gap` 702 → 1,549 |
| 5 | `Adversity` 가 표시 문자열을 컬럼에 직접 담았다 | `adversity_text` 로 분리 |

**②가 가장 컸다.** Prisma `Json?` 에 JS `null` 을 주면 SQL NULL 이 아니라
JSON `null` 이 쓰인다. `WHERE waves IS NULL` 이 0을 냈다 — **아카이브가 「없음」을
거짓말한다.** 계획 1에서 빈 파일 16개를 두고 `raw_file` 을 만든 것과 정면으로
어긋난다.

**⑤는 규약 문제였다.** 원본 `md__details.adversity` 는 `{name, desc, value}` 한
덩어리지만 이 저장소는 표시 문자열을 본체에서 떼어낸다(ADR-03). `Grace` 는
`GraceText` 로 지켰는데 `Adversity` 만 어겨 일관성이 깨졌다. 로케일이 셋이고
결손을 「행이 없다」로 표현하는 규약을 다른 계열과 맞췄다.

> **원본에 없는 구조를 우리가 만드는 결정이다.** 스펙의 「원본 전 필드 정규화」를
> 넘어 우리 규약을 원본에 얹었다. 사용자 승인을 받고 진행했다.
