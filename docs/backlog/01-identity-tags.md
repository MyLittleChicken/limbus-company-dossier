...
> 상태: **해소** (2026-08-06 · PR #27 앱 전환)
> 축을 둘로 나누기로 했으나, 실측 결과 **나눌 필요가 없다.** 남는 문제는 이름 하나다.
> 조사용 분류 시트가 저장소 루트 `tags-classify.csv` 에 있다(추적하지 않는다).

## 화면까지 닿았다 (2026-08-06)

앱 전환으로 정보 화면이 `canonical` 을 읽게 되면서 **화면에 실제로 반영됐다.**
골든 대조가 그것을 잡았다.

```
getCounts.affiliations   93 → 64      소속만 센다
인격 10208 소속           Blade Lineage · Syndicate → BLADE_LINEAGE 하나
                         Syndicate 는 소속이 아니라 특성 키워드였다
```

현행 `public` 의 `affiliation` 은 아직 있지만 화면이 안 읽는다. 그 표가 사라지는
것은 `DROP SCHEMA public` 때이고 M4 의 몫이다.
