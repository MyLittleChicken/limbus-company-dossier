...
**폴백을 쓰지 않는다.** 어느 출처에도 없으면 **행을 만들지 않고** `field_gap` 에
사유와 함께 남긴다(ADR-03 5절 원칙 그대로).

```
canonical.status_text 에 ko 행이 없다   →  한국어가 없다는 뜻이 명확하다
canonical.field_gap                    →  왜 없는지가 적혀 있다
build/gap-report.md                    →  채울 목록이 문서로 나온다
app.field_override                     →  채우면 재적재에도 살아남는다
```

실측 결손 **1,548건**. 소비 측이 폴백을 스스로 판정할 수 있다.
