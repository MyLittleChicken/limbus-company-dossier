...
export interface GiftVerdict {
	 ...
	 * 이 편성에서 켜질 수 있나.
	 *
	 * **막을 수 있는 근거(`Reason.blocking`)가 「미충족·확정」이면 false 다.**
	 * 게이트가 있는 기프트는 게이트만 막는다 — 적용 범위·수혜 대상은 확정
	 * 미충족이어도 죽이지 않는다. 게이트가 없는 기프트는 적용 범위를 뺀
	 * 나머지 전부가 막는다. 편성에 없는 것은 전투 중에도 안 생기므로(불충족은
	 * 언제나 확정) 막을 수 있는 조건이 미충족·확정이면 이 기프트는 이 편성에서
	 * 영영 안 켜진다.
	 *
	 * **이것이 조건 결합 문제의 출구다.** 「셋 중 둘이 맞았다」가 AND 인지 OR 인지는
	 * 데이터가 답하지 않지만(`gift_effect` 와 `gift_trigger` 를 잇는 표가 없다),
	 * 「막을 수 있는 조건 하나가 확정 불가라 못 켠다」는 결합을 몰라도 참이다.
	 *
