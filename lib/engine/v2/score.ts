 ...
 *
 * **저울추가 넷이다** — 확정 1.0 / 가능 0.5, 연쇄 1홉 1.0 / 2홉 0.5,
 * 전용 1.0 / 범용 0.5(이 파일 `fitOf`), 합성 도달 1.0 / 0.5 / 0.25(`fusion.ts`
 * `reachOf`). 넷 다 같은 규칙이고(한 단계 멀어지면 반) 나머지는 전부 실측값에서
 * 나온다. **이 헤더는 새 저울추를 만들지 않는다는 제약을 감사하는 자리다 —
 * 저울추를 늘릴 때는 이 수를 함께 고친다.**
 */
