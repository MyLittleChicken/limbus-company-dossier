...
**1단계(데이터베이스 구축) 진행 중 — 마무리 단계.** 제품의 정의와 범위는 [docs/00-product.md](docs/00-product.md)에 있다.
...
| 변환기 (원본 → 정규화 JSON) | 완료 — 47/47 테이블 |
| 적재기 (JSON → PostgreSQL) | 완료 — 49,977행 적재 |
| 검증 스크립트 | 완료 — 37건 통과 · 2건 미실행 |
| **잔여 결함** | **인격 스킬 12종이 정본에 없어 적재되지 않는다** |
| 2단계 (웹페이지 구축) | 미착수 |
...
`docs/00-product.md` 6절의 1단계 성공 기준은 "누락 없이 구축"을 요구하는데 **스킬 12종이 누락되어
아직 충족하지 못한다.** 변환기가 이를 실패로 판정하므로 `npm run convert`는 종료 코드 1을 반환한다.
...
- 원본 결손으로 채울 수 없는 것 — 코인 설명 912행 · 소속 한국어 12종 · 상태 이름 자리표시자 7종.
